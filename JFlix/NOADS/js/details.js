...
function playTrailer() {
  ...
  if (trailer) {
    ...
    if (popupModal && popupVideo) {
      // Create iframe for the trailer
      ...
      if (exitModalBtn) {
        exitModalBtn.addEventListener('click', closePopupTrailer);
      }
      
      // Add overlay for clicking outside to close
      const overlayClick = document.createElement('div');
      overlayClick.className = 'popup-overlay-click';
      overlayClick.onclick = closePopupTrailer;
      popupModal.appendChild(overlayClick);
...
function closePopupTrailer() {
  ...
  if (popupModal) {
    ...
    setTimeout(() => {
      ...
      // Remove overlay click element
      const overlayClick = popupModal.querySelector('.popup-overlay-click');
      if (overlayClick) {
        overlayClick.remove();
      }
