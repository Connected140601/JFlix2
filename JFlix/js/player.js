...
function loadVideo(id, type, season = null, episode = null) {
  ...
  // Apply sandboxing to block ads and pop-ups
  if (window.matchMedia('(display-mode: standalone)').matches) {
    videoPlayer.sandbox = 'allow-forms allow-pointer-lock allow-same-origin allow-scripts allow-top-navigation';
  } else {
...
// inside loadVideo before assigning src
// Apply CSP strict policy only for Server 5 (multiembed) to reduce ads
if (currentServer === 'multiembed') {
  videoPlayer.setAttribute('csp', "default-src 'self' https://multiembed.mov; frame-src https://multiembed.mov; img-src 'self' data:; style-src 'unsafe-inline'; script-src 'unsafe-inline';");
