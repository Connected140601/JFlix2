...
async function checkUserSession() {
    ...
    if (userToProcess) {
        ...
        updateUIForLoggedOutUser();
        // If on an account page and not logged in, show the login modal.
        const currentPath = window.location.pathname;
        if (currentPath.endsWith('/account.html')) {
            openModal('login-modal');
            showAuthMessage('login-modal', 'Please log in to access your account.', false);
        }
    }
        ...
        if (logoutButtonDropdown) {
            logoutButtonDropdown.addEventListener('click', async () => {
                ...
                if (error) {
                    ...
                    // Force a hard redirect to the homepage to bypass service worker issues
